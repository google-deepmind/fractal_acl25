...
"""XManager launch script for math reasoning experiments on PRM800K."""
import itertools
...
def main(_) -> None:
  with xm_local.create_experiment(experiment_title='experiment') as experiment:
    spec = xm.PythonContainer(
        ...
        entrypoint=xm.ModuleName('math_reasoning.main'),
    )
    ...
    parameters = [
        {
            ...
            'lambda_dllp': max(
                0, 1 - lambda_correlation_hp - lambda_cosine_similarity_hp
            ),
        ...
        for lr_hp, lambda_cosine_similarity_hp, lambda_correlation_hp, lf, lt in itertools.product(
            lr, lambda_cosine_similarity, lambda_correlation, loss_fn, loss_type
        )
    ]
