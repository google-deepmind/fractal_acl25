...
"""Utility functions for the math reasoning model."""
import tensorflow as tf
import tensorflow_probability as tfp


def loss_fn(loss_fn='mae'):  # pylint: disable=redefined-outer-name
  """Returns the loss function and the loss function with no reduction."""
  loss = None
  loss_no_red = None
  if loss_fn == 'mae':
...
def cosine_similarity_prior(
  ...
  Args:
    doc_segment_cosine_similarity: Cosine similarity between the document and
      summary segments.
    pred_summary_segment_scores: Predicted scores for the summary segments.
...
def correlation_prior(
  ...
  Args:
    summary_sentences_embedding_ragged: Embeddings of the summary sentences.
    pred_summary_segment_scores: Predicted scores for the summary segments.
    num_summary_segments: Number of summary segments.
...
def hybrid_prior(
  ...
  Args:
    instance_model_pred: Predicted scores for the instances.
    pred_summary_segment_scores: Predicted scores for the summary segments.
    num_summary_segments: Number of summary segments.
...
def get_dataloader_old(dataset_path, encoder_type='sentence-t5', batch_size=32):
  dataloaders = {}
  for split in ['train', 'test']:
    ds = tf.data.Dataset.load(f'{dataset_path}/{encoder_type}_{split}_phase2')
    ds = ds.shuffle(len(ds))
    if split == 'train':
      ds = ds.take(int(0.5*len(ds)))
...
def get_dataloader(
  ...
  """Returns the dataloader for the given dataset path.
...
def get_dataloader_hybrid_prior(
  ...
  Args:
    ...
    encoder_type: Type of the encoder.
    batch_size: Batch size.
    frac_disagg_bags: Fraction of disaggregated bags.
    cv_split: Cross-validation split.
